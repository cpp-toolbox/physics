...
void Physics::load_model_into_physics_world(Model *model) {
  ...
  for (int i = 0; i < model->meshes.size(); i++) {
    ...
    JPH::Body *mesh_body =
        body_interface.CreateBody(mesh_settings); // Note that if we run out of bodies this can return nullptr
...
void Physics::create_character() {
  ...
  settings->mSupportingVolume = JPH::Plane(
      JPH::Vec3::sAxisY(), -this->character_radius); // Accept contacts that touch the lower sphere of the capsule
...
void Physics::update(float delta_time) {
  ...
  // std::cout << "Character UP = (" << ch_up.GetX() << ", " << ch_up.GetY() << ", " << ch_up.GetZ() << ")" <<
  // std::endl;
